Deface::Override.new(
  ...
  text: %q{
    <div class="inner" data-hook=<%="#{address_type}_inner" %>>
      ...
      <p class="form-group" id=<%="#{address_id}taxid" %>>
        <%= form.label :taxid, Spree.t(:taxid) %>
        <%= form.text_field :taxid, class: 'form-control' %>
      </p>
      ...
      <p class="form-group" id=<%="#{address_id}zipcode" %>>
        ...
        <% end %>
        <%= form.text_field :zipcode, class: "form-control #{'required' if address.require_zipcode?}",:data => {:addressid => address_id } %>
      </p>
